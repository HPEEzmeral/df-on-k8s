...
BOOTSTRAP_BUILD_VERSION_NO = "1.5.2.0"
...
class BootstrapBase(object):
    ...
    def _parse_args(self):
        ...
        self.arg_parser.add_argument("-m", "--mode", action="store", default=self.prompt_mode,
                                     help="prompt mode ({0}, {1}, {2})".format(Prompts.PROMPT_MODE_STR,
                                                                               Prompts.HEADLESS_MODE_STR,
                                                                               Prompts.RECORD_MODE_STR))
        self.arg_parser.add_argument("-r", "--response-file", action="store", default=self.prompt_response_file, help="prompt response file")
        ...
        self.arg_parser.add_argument("--no_python_check", action="store_true", default=False, help="Do not stop execution for an invalid Python: NOT RECOMMENDED")

        if self.run_type == BootstrapBase.INSTALL:
            self.arg_parser.add_argument("--setup_only", action="store_true", default=False, help="Perform virtualenv install only")
            self.arg_parser.add_argument("--drill_install", action="store_true", default=False, help=argparse.SUPPRESS)
            self.arg_parser.add_argument("--openshift_install", action="store_true", default=False, help=argparse.SUPPRESS)
            ...
            self.arg_parser.add_argument("--cloud_create", action="store_true", default=False, help=argparse.SUPPRESS)
            self.arg_parser.add_argument("--core_install", action="store_true", default=False, help=argparse.SUPPRESS)
            self.arg_parser.add_argument("--dataplatform_operator", action="store_true", default=False, help=argparse.SUPPRESS)
        elif self.run_type == BootstrapBase.UNINSTALL:
            self.arg_parser.add_argument("--drill_uninstall", action="store_true", default=False, help=argparse.SUPPRESS)
            self.arg_parser.add_argument("--openshift_uninstall", action="store_true", default=False, help=argparse.SUPPRESS)
    ...
    def prologue(self):
        ...
        Log.info(title, True)
        Log.info("Copyright 2022 Hewlett Packard Enterprise (HPE), All Rights Reserved", True)
    ...
    def python_check(self):
        python_validator = PythonValidator(self.parsed_args.no_python_check)
        ...
        if python_validator.operation == Validator.OPERATION_WARNING:
            if not self.parsed_args.no_python_check:
    ...
    def check_is_helm_installed():
        response, status = OSCommand.run2("helm version --short --client")
        if status != 0:
            Log.error("Cannot connect to Helm. Make sure helm is installed on the machine.")
            return False

        lines = response.strip(os.linesep).split(os.linesep)
        for line in lines:
            line = line.lower()
            # ignoring warnings if there are any
            if not line.startswith("warning"):
                # checking if version matches with format vx.x.x
                match = re.search("v\\d+.\\d+.\\d+", line)
                if match is None or len(match.string) <= 0:
                    Log.error("Could not parse helm version from version string: {0}".format(line))
                    return False
                # check if version number starts with v3
                version_num_breakdown = line.split(".")
                if version_num_breakdown[0] != "v3":
                    Log.error("Installing spark depends on help version 3.0.0 or greater. Your version: {0} is not "
                              "supported currently".format(line))
                    return False
        return True
