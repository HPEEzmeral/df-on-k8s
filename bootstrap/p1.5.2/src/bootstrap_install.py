...
class BootstrapInstall(BootstrapBase):
    ...
    def run(self):
        ...
        if dataplatform.dataplatform_operation(self.parsed_args, True):
            return

        if not self.core_install_enabled:
            self.core_install_enabled = self.parsed_args.core_install

        do_cloud_install = False
        if self.parsed_args.cloud_install or self.parsed_args.cloud_create:
            do_cloud_install = True
        ...
        if self.parsed_args.cloud_create:
            BootstrapBase.exit_application(0)
        ...
        tolerate_master = self.check_if_tolerate_master()
        str_tolerations = ""
        if tolerate_master:
        ...
        if do_fakelabels:
            fake_labels = self.check_if_fakelabels(install_storage)
            if not fake_labels:
                OperationsBase.fake_labels = "false"
            else:
                fake_labels_disk_list = self.check_if_fakelabels_disk_list()
                OperationsBase.replace_dict["{fake-labels-disk-lists}"] = fake_labels_disk_list
        ...
        if install_csi:
            snapshotter_crds.install_snapshottercrd_components()
    ...
    def check_if_fakelabels(self, default):
        print(os.linesep)
        fake = self._prompts.prompt_boolean("Label nodes without Ezmeral Container Platform assistance? Note: This is unsupported!", default, key_name="FAKE_LABELS")
        return fake

    def check_if_fakelabels_disk_list(self):
        print(os.linesep)
        disk_list = self._prompts.prompt("Hard code disk list? Note: This is unsupported!", default="", key_name="FAKE_LABELS_DISK_LIST")
        return disk_list
    ...
    def check_if_tolerate_master(self):
        choice = self._prompts.prompt_boolean("Schedule Data Fabric pods on master?", False, key_name="TOLERATE_MASTER")
        return choice
