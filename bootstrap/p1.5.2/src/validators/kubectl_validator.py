...
class KubectlValidator(Validator):
    KUBERNETES_CLIENT_MIN = Version(1, 19, 0)
    _instance = None
    ...
    def collect(self):
        ...
        #    return
        versions = []
        versions.append(response.split("Client Version: ")[1].split("\n")[0])
        versions.append(response.split("Server Version: ")[1].split("\n")[0])
        ...
        if self.client_version < self.KUBERNETES_CLIENT_MIN:
            Log.error("The kubectl client version {0} is less than the required minimum {1}".format(self.client_version, self.KUBERNETES_CLIENT_MIN))
            self.operation = Validator.OPERATION_INSTALL
            return
