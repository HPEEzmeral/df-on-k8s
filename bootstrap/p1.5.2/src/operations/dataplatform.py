...
class DataPlatform(OperationsBase):
    ...
    def load_yaml_dict(self):
        ...
        file_name = self.check_exists(self.system_dataplatform, "autogenerated-system-crd-dataplatformoperator.yaml")
    ...
    def dataplatform_operation(self, args, install):
        if args.dataplatform_operator:
            found = False
            for yaml in self.yamls:
                if yaml.file_id == "system-dataplatformoperator":
                    found = True
                    OperationsBase.replace_dict["{operator-repo}"] = OperationsBase.operator_repo
                    if install:
                        rslt = self.run_kubectl_apply(yaml)
                        if rslt:
                            Log.info("Dataplatform operator was installed", stdout=True)
                    else:
                        rslt = self.run_kubectl_delete(yaml)
                        if rslt:
                            Log.info("Dataplatform operator was uninstalled", stdout=True)
                    break

            if not found:
                Log.error("Could not find system-dataplatformoperator")
