...
class LDAP(OperationsBase):
    ...
    def load_yaml_dict(self):
        ...
        ldap_seed_secret_yaml_file = YamlFile("ldap-seed-secret", "LDAP secret for Example LDAP, file_name", file_name, "ldap_components")
    ...
    def raw_linux_users(self):
        ...
        self.admin_password = self.prompts.prompt("Admin user account password", self.admin_password, True, key_name="ADMIN_PASSWORD")
    ...
    def example_ldap(self):
        print("")
        Log.info(os.linesep + "An example LDAP service will be created in the hpe-ldap namespace.  The admin account "
                              "that will be created will be \'admin/admin123\'.  Some sample users and a read only "
                              "user will also be created.", True)
